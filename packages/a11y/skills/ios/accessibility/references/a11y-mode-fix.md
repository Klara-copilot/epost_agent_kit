...
Activated by: `/ios:a11y:fix <id>` and `/ios:a11y:fix-batch <n>` commands.
...
1. **Check existing patches** — Look in `.epost-data/a11y/fixes/patches/` for `finding-{id}-*.diff`. If a patch exists, review it for applicability before generating a new one
...
8. **Suggest close** — After fix is applied and verified, suggest: `Run /ios:a11y:close {id} to mark as resolved`
