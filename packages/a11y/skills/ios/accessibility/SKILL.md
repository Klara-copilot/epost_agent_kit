...
Projects using this skill can maintain a `known-findings.json` file at `.epost-data/a11y/known-findings.json` (see `known-findings-schema.json` for format) tracking discovered accessibility violations. The `epost-a11y-specialist` uses this file in fix mode to apply surgical fixes.
