---
name: media-ai
description: "Use when: analyzing audio/video files, processing images or screenshots, extracting data from PDFs, generating images from text prompts, or implementing multimodal AI features. Powered by Google Gemini API — audio up to 9.5h, video up to 6h, documents up to 1,000 pages, image generation, OCR, transcription with timestamps. Supports Gemini 2.5/2.0 with up to 2M token context."
user-invocable: true
context: inline
---
...
2. Project root: `.env`
3. `packages/core/skills/media-ai/.env`
...
```bash
.claude/skills/.venv/bin/pip install -r packages/core/skills/media-ai/scripts/requirements.txt
```

Invoke scripts using the venv interpreter:

```bash
.claude/skills/.venv/bin/python3 packages/core/skills/media-ai/scripts/gemini_batch_process.py --help
```
