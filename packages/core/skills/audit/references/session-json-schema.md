...
description: "(ePost) Schema for session.json — per-audit-session metadata file written to every session folder"
...
| `agents[].name` | `string` | Yes | Agent identifier (e.g. `epost-muji`) |
