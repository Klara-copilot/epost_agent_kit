...
function findSkillCreatorDir(cwd) {
  const candidates = [
    ...
    path.join(cwd, 'packages', 'core', 'skills', 'skill-creator'),
  ];
...
function printUsage() {
  ...
  console.log('  npm run skill:eval-all');
  console.log('  npm run skill:eval-all -- --filter "web-*"');
  console.log('  npm run skill:eval-all -- --model claude-haiku-4-5-20251001 --concurrency 2');
}
...
if (!python) {
  console.error('Error: python3 not found. Install via: brew install python');
  process.exit(1);
}

const skillCreatorDir = findSkillCreatorDir(cwd);
if (!skillCreatorDir) {
  console.error('Error: skill-creator not found at .claude/skills/skill-creator');
  process.exit(1);
}
