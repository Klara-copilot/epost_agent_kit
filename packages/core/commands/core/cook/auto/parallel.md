...
### 1. Research (Optional)
- Use max 2 `epost-researcher` agents in parallel if tasks complex
- Use `epost-scout` to search codebase
...
### 4. Testing
- Use `epost-tester` subagent for full test suite
- NO fake data/mocks/cheats
- If fail: use `epost-debugger`, fix, repeat

### 5. Code Review
- Use `epost-reviewer` for all changes
- If critical issues: fix, retest

### 6. Project Management & Docs
- If approved: use `epost-orchestrator` + `epost-documenter` in parallel
...
**Example:** Phases 1-3 parallel → Launch 3 epost-implementer agents → Wait → Phase 4 sequential
