...
**File**: `.github/skills/skill-index.json`

**Find by keyword**:
```bash
jq '.skills[] | select(.keywords[] | contains("debug"))' .github/skills/skill-index.json
```

**Find by trigger**:
```bash
jq '.skills[] | select(.triggers[] | contains("/debug"))' .github/skills/skill-index.json
```

**Find by agent**:
```bash
jq '.skills[] | select(.["agent-affinity"][] | contains("epost-implementer"))' .github/skills/skill-index.json
```

**Get skill path**:
```bash
jq -r '.skills[] | select(.name == "debugging") | .path' .github/skills/skill-index.json
```
...
```bash
# List aspect files
ls .github/skills/debugging/references/

# Read aspect
cat .github/skills/debugging/references/root-cause-analysis.md
```
