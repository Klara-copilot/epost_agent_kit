 ...
 * Run: node --test .github/hooks/__tests__/dev-rules-reminder.test.cjs
 *
...
describe('dev-rules-reminder.cjs', () => {
  ...
  describe('Issue #327: Path Resolution', () => {
    ...
    it('uses CWD as base for paths, not git root', async () => {
      ...
      // Use .github/hooks as subdirectory
      ...
      if (result.stdout && result.stdout.includes('Reports:')) {
        ...
        assert.ok(
          result.stdout.includes(subdirPath) || result.stdout.includes('.github/hooks'),
