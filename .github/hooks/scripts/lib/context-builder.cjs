...
function resolveRulesPath(filename, configDirName = '.claude') {
  ...
  if (fs.existsSync(globalRulesPath)) return `~/.github/rules/${filename}`;
  ...
  if (fs.existsSync(globalWorkflowsPath)) return `~/.github/workflows/${filename}`;
...
function resolveScriptPath(filename, configDirName = '.claude') {
  ...
  if (fs.existsSync(globalPath)) return `~/.github/scripts/${filename}`;
  return null;
}
...
function resolveSkillsVenv(configDirName = '.claude') {
  ...
  if (fs.existsSync(globalVenv)) {
    return isWindows
      ...
      : '~/.github/skills/.venv/bin/python3';
  }
  return null;
}
...
function buildRulesSection({ devRulesPath, catalogScript, skillsVenv }) {
  ...
  if (skillsVenv) {
    lines.push(`- Python scripts in .github/skills/: Use \`${skillsVenv}\``);
  }
