 ...
 * Run: node --test .github/hooks/lib/__tests__/context-builder.test.cjs
 *
...
describe('context-builder.cjs', () => {
  ...
  describe('resolveRulesPath()', () => {
    ...
    it('returns null when file does not exist anywhere', () => {
      ...
      // Use a unique filename that won't exist in global ~/.github/
    ...
    it('finds file in rules/ directory (new location)', () => {
      tempDir = createTempDir(['.github/rules']);
      createTestFile(path.join(tempDir, '.github/rules'), 'development-rules.md');
      process.chdir(tempDir);

      const result = contextBuilder.resolveRulesPath('development-rules.md');
      assert.strictEqual(result, '.github/rules/development-rules.md',
        'Should find file in rules/ directory');
    });

    it('falls back to workflows/ when rules/ does not exist', () => {
      tempDir = createTempDir(['.github/workflows']);
      createTestFile(path.join(tempDir, '.github/workflows'), 'development-rules.md');
      process.chdir(tempDir);

      const result = contextBuilder.resolveRulesPath('development-rules.md');
      assert.strictEqual(result, '.github/workflows/development-rules.md',
        'Should fall back to workflows/ directory');
    });

    it('prefers rules/ over workflows/ when both exist', () => {
      tempDir = createTempDir(['.github/rules', '.github/workflows']);
      createTestFile(path.join(tempDir, '.github/rules'), 'development-rules.md', '# Rules version\n');
      createTestFile(path.join(tempDir, '.github/workflows'), 'development-rules.md', '# Workflows version\n');
      process.chdir(tempDir);
    ...
    it('finds file in workflows/ when rules/ exists but file is only in workflows/', () => {
      ...
      createTestFile(path.join(tempDir, '.github/workflows'), 'legacy-file.md');
      process.chdir(tempDir);

      const result = contextBuilder.resolveRulesPath('legacy-file.md');
      assert.strictEqual(result, '.github/workflows/legacy-file.md',
  ...
  describe('resolveWorkflowPath alias', () => {
    ...
    it('resolveWorkflowPath works identically to resolveRulesPath', () => {
      const tempDir = createTempDir(['.github/rules']);
      createTestFile(path.join(tempDir, '.github/rules'), 'test.md');
  ...
  describe('Global path resolution', () => {
    ...
    it('checks global ~/.github/rules/ path', () => {
      ...
      try {
        ...
        // Result depends on whether global ~/.github/rules exists
  ...
  describe('CLAUDE.md reference resolution', () => {
    ...
    it('resolves @rules/ references correctly', () => {
      ...
      tempDir = createTempDir(['.github/rules']);
      createTestFile(path.join(tempDir, '.github/rules'), 'primary-workflow.md');
      createTestFile(path.join(tempDir, '.github/rules'), 'development-rules.md');
      createTestFile(path.join(tempDir, '.github/rules'), 'orchestration-protocol.md');
      createTestFile(path.join(tempDir, '.github/rules'), 'documentation-management.md');
      process.chdir(tempDir);
    ...
    it('resolves legacy @workflows/ references via fallback', () => {
      ...
      tempDir = createTempDir(['.github/workflows']);
      createTestFile(path.join(tempDir, '.github/workflows'), 'primary-workflow.md');
      process.chdir(tempDir);
