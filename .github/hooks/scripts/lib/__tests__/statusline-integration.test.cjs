 ...
 * Run: node .github/hooks/lib/__tests__/statusline-integration.test.cjs
 */
...
try {
  const result = execSync(`echo '${minimalInput.replace(/'/g, "'\\''")}'  | node .github/statusline.cjs`, {
    encoding: 'utf8',
...
try {
  ...
  const gitResult = execSync(`echo '${gitInput.replace(/'/g, "'\\''")}'  | node .github/statusline.cjs`, {
    encoding: 'utf8',
    cwd: tmpDir,
    stdio: ['pipe', 'pipe', 'pipe']
  });
...
try {
  const contextResult = execSync(`echo '${contextInput.replace(/'/g, "'\\''")}'  | node .github/statusline.cjs`, {
    encoding: 'utf8',
    stdio: ['pipe', 'pipe', 'pipe']
  });
...
try {
  const costResult = execSync(`echo '${costInput.replace(/'/g, "'\\''")}'  | CLAUDE_BILLING_MODE=api node .github/statusline.cjs`, {
    encoding: 'utf8',
    stdio: ['pipe', 'pipe', 'pipe'],
...
try {
  const invalidResult = execSync(`echo 'not valid json'  | node .github/statusline.cjs`, {
    encoding: 'utf8',
    stdio: ['pipe', 'pipe', 'pipe']
  });
...
try {
  const emptyResult = execSync(`echo '' | node .github/statusline.cjs`, {
    encoding: 'utf8',
    stdio: ['pipe', 'pipe', 'pipe']
  });
...
try {
  const multilineResult = execSync(`echo '${multilineInput.replace(/'/g, "'\\''")}'  | node .github/statusline.cjs`, {
    encoding: 'utf8',
    stdio: ['pipe', 'pipe', 'pipe']
  });
...
try {
  const expandResult = execSync(`echo '${expandInput.replace(/'/g, "'\\''")}'  | node .github/statusline.cjs`, {
    encoding: 'utf8',
    stdio: ['pipe', 'pipe', 'pipe']
  });
...
try {
  ...
  const noColorResult = execSync(`echo '${colorInput.replace(/'/g, "'\\''")}'  | NO_COLOR=1 node .github/statusline.cjs`, {
    encoding: 'utf8',
    stdio: ['pipe', 'pipe', 'pipe'],
...
let wideLines = 0;
try {
  const wideResult = execSync(`echo '${wideInput.replace(/'/g, "'\\''")}'  | COLUMNS=160 node .github/statusline.cjs`, {
    encoding: 'utf8',
    stdio: ['pipe', 'pipe', 'pipe'],
...
try {
  const narrowResult = execSync(`echo '${narrowInput.replace(/'/g, "'\\''")}'  | COLUMNS=80 node .github/statusline.cjs`, {
    encoding: 'utf8',
    stdio: ['pipe', 'pipe', 'pipe'],
...
try {
  const longPathResult = execSync(`echo '${longPathInput.replace(/'/g, "'\\''")}'  | COLUMNS=100 node .github/statusline.cjs`, {
    encoding: 'utf8',
    stdio: ['pipe', 'pipe', 'pipe'],
...
try {
  const longModelResult = execSync(`echo '${longModelInput.replace(/'/g, "'\\''")}'  | COLUMNS=100 node .github/statusline.cjs`, {
    encoding: 'utf8',
    stdio: ['pipe', 'pipe', 'pipe'],
    env: { ...process.env, COLUMNS: '100' }
  });
...
try {
  const agentTodoResult = execSync(`echo '${agentTodoInput.replace(/'/g, "'\\''")}'  | node .github/statusline.cjs`, {
    encoding: 'utf8',
    stdio: ['pipe', 'pipe', 'pipe']
  });
...
try {
  const zeroResult = execSync(`echo '${zeroContextInput.replace(/'/g, "'\\''")}'  | node .github/statusline.cjs`, {
    encoding: 'utf8',
    stdio: ['pipe', 'pipe', 'pipe']
  });
...
try {
  const tinyResult = execSync(`echo '${wideInput.replace(/'/g, "'\\''")}'  | COLUMNS=40 node .github/statusline.cjs`, {
    encoding: 'utf8',
    stdio: ['pipe', 'pipe', 'pipe'],
