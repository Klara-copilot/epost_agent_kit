...
Authoritative, enforceable rules for general code reviews. Each rule has a unique ID, severity, pass criterion, and fail criterion. Used by epost-code-reviewer during `/review` and `/audit --code` to evaluate backend services, API layers, shared utilities, and web logic outside UI components.
