---
name: knowledge
description: "(ePost) Use when retrieving prior decisions, internal patterns, or project context. NOT for external library how-tos. Also use to capture learnings after completing a non-trivial task."
user-invocable: false

metadata:
  agent-affinity: [epost-planner, epost-fullstack-developer, epost-debugger, epost-researcher, epost-code-reviewer]
  keywords: [retrieve, search, knowledge, context, rag, lookup, prior-art, capture, learn, persist, record, context7, api-reference, library, documentation]
  platforms: [all]
  triggers: ["what do we know about", "check knowledge", "prior art", "previous decision", "capture learnings", "save pattern", "record finding", "api reference", "library docs", "how to use"]
  connections:
    enhances: [plan, debug]
---
...
Internal-first knowledge retrieval and post-task capture. Retrieval: search docs/, RAG, skills, codebase before going external. Capture: persist learnings to `docs/` for team-wide reuse.
...
| `--external` | `references/external-docs.md` | Fetching API/library docs via Context7 or WebSearch |
...
3. **Skip irrelevant levels**: No RAG server? Skip L2, go directly to L4 — never block on RAG availability
...
9. **Update knowledge**: If external search yields new insight, use `--capture`

## Aspect Files

| File | Purpose |
|------|---------|
| `references/search-strategy.md` | How to search each source |
| `references/priority-matrix.md` | Decision table for source priority |
| `references/knowledge-base.md` | Knowledge system structure, document types, ADR templates |
| `references/capture.md` | Post-task capture workflow — persist learnings to docs/ |
| `references/external-docs.md` | Context7 + WebSearch workflow for library/API docs |

## Related Skills

- `docs` — Initialize and maintain docs/ registry
- `repomix` — Pack repo contents for LLM context
