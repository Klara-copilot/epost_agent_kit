...
Use `epost-orchestrator` agent to read plan file completely. Map dependencies between tasks. List ambiguities or blockers. Identify required skills/tools and activate from catalog. Parse phase file and extract actionable tasks.

**TodoWrite Initialization & Task Extraction:**
`epost-orchestrator` agent must respond back with:
...
Write tests covering happy path, edge cases, and error cases. Call `epost-tester` subagent: "Run test suite for plan phase [phase-name]". If ANY tests fail: STOP, call `epost-debugger` subagent: "Analyze failures: [details]", fix all issues, re-run `epost-tester`. Repeat until 100% pass.
...
Call `epost-reviewer` subagent: "Review code changes in **Step 2** of plan phase [phase-name]. Check security, performance, architecture, YAGNI/KISS/DRY. Return score (X/10), critical issues list, warnings list, suggestions list."
...
cycle = 0
LOOP:
  1. Run epost-reviewer → get score, critical_count, warnings, suggestions
  ...
  4. ELSE IF critical_count > 0 AND cycle < 3:
     ...
     → Re-run epost-tester to verify no regressions
     → cycle++, GOTO LOOP
...
- **Call** `epost-orchestrator` sub-agent: "Update plan status in [plan-path]. Mark plan phase [phase-name] as DONE with timestamp. Update roadmap."
- **Call** `epost-documenter` sub-agent: "Update docs for plan phase [phase-name]. Changed files: [list]."
...
- **Call** `epost-git-manager` subagent to handle git operation.
...
- Step 3: `epost-tester`
- Step 4: `epost-reviewer`
- Step 5: `epost-orchestrator` AND `epost-documenter` AND `epost-git-manager`
