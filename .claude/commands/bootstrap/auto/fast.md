...
**First thing first:** check if Git has been initialized, if not, use `epost-git-manager` subagent to quickly initialize it (use `main` branch).
...
* Use 2 `epost-researcher` subagents in parallel (only read up to max 5 sources) to explore the user's request, idea validation, challenges, and find the best possible solutions.
* Use 2 `epost-researcher` subagents in parallel (only read up to max 5 sources) to find a best fit tech stack for this project.
* Use 2 `epost-researcher` subagents in parallel (only read up to max 5 sources) to create a design plan that follows the progressive disclosure structure:
...
* Use `epost-ui-ux-designer` subagent to analyze the research results and create the design guidelines at `./docs/design-guidelines.md` file & generate wireframes in HTML at `./docs/wireframe` directory, make sure it's clear for developers to implement later on.
...
* Use `epost-architect` subagent to analyze all reports and create the detailed step by step implementation plan at `./plans` directory following the progressive disclosure structure above.
* **IMPORTANT:** Sacrifice grammar for the sake of concision when writing outputs.

### Implementation

* Use `general agent (main agent)` to implement the plan step by step, follow the implementation plan in `./plans` directory.
* Use `epost-ui-ux-designer` subagent to implement the frontend part follow the design guidelines at `./docs/design-guidelines.md` file.
...
* Use `epost-tester` subagent to run the tests, make sure all tests pass and the app is working, then report back to main agent.
* If there are issues or failed tests, use `epost-debugger` subagent to find the root cause of the issues, then ask main agent to fix all of them.
...
* After finishing, delegate to `epost-reviewer` subagent to review code. If there are critical issues, ask main agent to improve the code and tell `epost-tester` agent to run the tests again. Repeat the process until all tests pass.
...
* Use `epost-documenter` subagent to update the docs if needed.
  ...
  * **IMPORTANT:** Sacrifice grammar for the sake of concision when writing outputs.
* Use `epost-orchestrator` subagent to create a project roadmap at `./docs/project-roadmap.md` file.

### Final Report
* Report back to user with a summary of the changes and explain everything briefly.
* Use `epost-git-manager` subagent to create commits for the implemented changes (DO NOT push to remote repository).
* **IMPORTANT:** Sacrifice grammar for the sake of concision when writing reports.
