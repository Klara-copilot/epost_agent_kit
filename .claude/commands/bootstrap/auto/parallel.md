...
### 1. Git Init
- Check if Git initialized, if not: use `epost-git-manager` (main branch)

### 2. Research
- Use max 2 `epost-researcher` agents in parallel
- Explore requirements, validation, challenges, solutions
- Keep reports ≤150 lines

### 3. Tech Stack
- Use `epost-architect` + multiple `epost-researcher` agents in parallel for best fit tech stack
- Write to `./docs` directory (≤150 lines)

### 4. Wireframe & Design
- Use `epost-ui-ux-designer` + `epost-researcher` agents in parallel
...
- Launch multiple `epost-implementer` agents in PARALLEL for concurrent phases
  - Pass: phase file path, environment info
- Use `epost-ui-ux-designer` for frontend (generate/analyze assets with `ai-multimodal`, edit with `imagemagick`)
- Run type checking after implementation

### 6. Testing
- Write real tests (NO fake data/mocks)
- Use `epost-tester` subagent
- If fail: `epost-debugger` → fix → repeat
...
### 8. Documentation
- Use `epost-documenter` to create/update:
  - `./docs/README.md` (≤300 lines)
  - `./docs/project-overview-pdr.md`
  - `./docs/code-standards.md`
  - `./docs/system-architecture.md`
- Use `epost-orchestrator` for `./docs/project-roadmap.md`
...
- Ask to commit (use `epost-git-manager` if yes)
